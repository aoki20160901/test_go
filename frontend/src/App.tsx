import React, { useState } from "react";

interface Photo {
  file: File | null;
  preview: string;
}

interface PhotoUploadProps {
  label: string;
  photos: Photo[];
  setPhotos: (photos: Photo[]) => void;
}

function PhotoUpload({ label, photos, setPhotos }: PhotoUploadProps) {
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    if (!e.target.files) return;
    const file = e.target.files[0];
    const reader = new FileReader();
    reader.onloadend = () => {
      const updated = [...photos];
      updated[index] = { file, preview: reader.result as string };
      setPhotos(updated);
    };
    reader.readAsDataURL(file);
  };

  return (
    <div style={{ marginBottom: "1.5rem", width: "100%" }}>
      <h2 style={{ fontSize: "1.25rem", fontWeight: "bold", marginBottom: "0.5rem" }}>{label}</h2>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem" }}>
        {photos.map((p, i) => (
          <div
            key={i}
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              gap: "0.5rem",
              padding: "0.75rem",
              border: "1px solid #ccc",
              borderRadius: "8px",
              width: "220px",
            }}
          >
            <input type="file" accept="image/*" onChange={(e) => handleFileChange(e, i)} />
            {p.preview ? (
              <img src={p.preview} alt="preview" style={{ width: 160, height: 160, objectFit: "cover" }} />
            ) : (
              <span>プレビューなし</span>
            )}
          </div>
        ))}
      </div>
    </div>
  );
}

export default function App() {
  const [entrancePhotos, setEntrancePhotos] = useState<Photo[]>([
    { file: null, preview: "" },
    { file: null, preview: "" },
  ]);
  const [hallwayPhotos, setHallwayPhotos] = useState<Photo[]>([
    { file: null, preview: "" },
    { file: null, preview: "" },
  ]);
  const [comment, setComment] = useState("");

  const handleReport = async () => {
    alert("レポート生成（バックエンド接続してください）");
  };

  return (
    <div
      style={{
        display: "flex",
        flexDirection: "column",
        alignItems: "center",  // ← これで画面中央揃え
        gap: "1rem",
        padding: "1rem",
      }}
    >
      <PhotoUpload label="玄関の写真" photos={entrancePhotos} setPhotos={setEntrancePhotos} />
      <PhotoUpload label="廊下の写真" photos={hallwayPhotos} setPhotos={setHallwayPhotos} />

      <div style={{ width: "100%", maxWidth: "500px", marginBottom: "1rem" }}>
        <h2 style={{ fontSize: "1.25rem", fontWeight: "bold", marginBottom: "0.5rem" }}>総評</h2>
        <textarea
          value={comment}
          onChange={(e) => setComment(e.target.value)}
          rows={4}
          style={{ width: "100%", padding: "0.5rem", border: "1px solid #ccc", borderRadius: "4px" }}
          placeholder="総評を入力してください"
        />
      </div>

      <button
        onClick={handleReport}
        style={{
          padding: "0.75rem 1.5rem",
          cursor: "pointer",
          backgroundColor: "#2563eb",
          color: "white",
          borderRadius: "6px",
        }}
      >
        レポート生成
      </button>
    </div>
  );
}
